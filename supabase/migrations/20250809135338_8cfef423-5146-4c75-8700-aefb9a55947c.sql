...
create index idx_horoscopes_sign_period_dates on public.horoscopes(sign, period, date_start desc);
...
create table public.birth_chart_submissions (
  id uuid primary key default gen_random_uuid(),
  user_id uuid,
  name text,
...
create policy "Users can view their own admin email"
...
create policy "Everyone can read horoscopes"
  on public.horoscopes
  for select
  using (true);

create policy "Admins can insert horoscopes"
  on public.horoscopes
  for insert
  to authenticated
  with check (public.is_admin());

create policy "Admins can update horoscopes"
  on public.horoscopes
  for update
  to authenticated
  using (public.is_admin());

create policy "Admins can delete horoscopes"
  on public.horoscopes
  for delete
  to authenticated
  using (public.is_admin());
...
create policy "Everyone can read compatibility"
...
create policy "Anyone can submit birth charts"
  on public.birth_chart_submissions
  for insert
  with check (true);

create policy "Admins can read birth chart submissions"
  on public.birth_chart_submissions
  for select
  to authenticated
  using (public.is_admin());

create policy "Admins can delete birth chart submissions"
  on public.birth_chart_submissions
  for delete
  to authenticated
  using (public.is_admin());
